...
class Agent {
  ...
  connect() {
    ...
    var options = {
      url: (app.get("hub_url") || knowledge_url) + 'knowledge/message-hub-credentials',
      method: 'get',
      headers: headers
    };
      ...
      request(options, function (err, response, body) {
        if (err || response.statusCode !== 200) {
          ...
          var driver_options = {
            ...
            'security.protocol': 'sasl_ssl',
            //'ssl.ca.location': '/etc/ssl/certs', // this is the default location in bluemix - uncomment for bluemix upload
            'sasl.mechanisms': 'PLAIN',
  ...
  subscribe() {
    ...
    console.log("Created agent with event " + agent.event);
    var stream = Kafka.KafkaConsumer.createReadStream(agent.connection_options, {}, {
      topics: [agent.event]
    });
    agent.stream = stream;
